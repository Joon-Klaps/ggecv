...
##' @export
draw_key_ecv <- function(data, params, size) {

  filename <- system.file(paste0(data$ecv, ".png"), package = "ggecvs")
...
##' @param nudge_x horizontal adjustment to nudge ecvs
...
##'
geom_ecv <- function(mapping = NULL, data = NULL, stat = "identity",
...
GeomEcv <- ggplot2::ggproto("GeomEcv", ggplot2::Geom,
                     ...
                     default_aes = ggplot2::aes(ecv = "bv", size = 1,
                     ...
                     draw_panel = function(data, panel_params, coord, by, na.rm=FALSE,
                       ...
                       if (!is.null(.fun) && is.function(.fun)) {
                         data$ca <- .fun(data$ecv)
                       }
                       if (is.null(data$ecv)) return(NULL)

                       groups <- split(data, factor(data$ecv))
                       imgs <- names(groups)
                       ...
                       ggplot2:::ggname("geom_ecv",
                              gTree(children = grobs, cl = "fixasp_raster"))
                     },
                     non_missing_aes = c("size", "ecv"),
                     required_aes = c("x", "y"),
                     draw_key = draw_key_ecv ## draw_key_blank ## need to write the `draw_key_ecv` function.
)
...
imageGrob <- function(x, y, size, img, by, hjust, colour, alpha, image_fun, angle, asp = 1) {
  if (!methods::is(img, "magick-image")) {
      filename <- system.file(paste0(img, ".png"), package = "ggecvs")
